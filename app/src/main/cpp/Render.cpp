...
void Render::drawFrame() {
    ...
    if (vkQueueSubmit(graphicsQueue, 1, &computeSubmitInfo, computeInFlightFences[currentFrame]) != VK_SUCCESS) {
        LOGI("failed to submit compute command buffer!");
    }
...
void Render::createFramebuffers() {
    ...
    for (size_t i = 0; i < swapChainImageViews.size(); i++) {
        ...
        if (vkCreateFramebuffer(device, &framebufferInfo, nullptr, &swapChainFramebuffers[i]) != VK_SUCCESS) {
            LOGI("Failed to create framebuffer!");
        }
    }
...
void Render::createCommandBuffer() {
    ...
    if (vkAllocateCommandBuffers(device, &allocInfo, commandBuffers.data()) != VK_SUCCESS) {
        LOGI("Failed to allocate command buffers!");
    }
...
void Render::recordCommandBuffer(VkCommandBuffer commandBuffer, uint32_t imageIndex) {
    ...
    if (vkEndCommandBuffer(commandBuffer) != VK_SUCCESS) {
        LOGI("Failed to record command buffer!");
    }
}
...
void Render::createComputeDescriptorSetLayout() {
    ...
    if (vkCreateDescriptorSetLayout(device, &layoutInfo, nullptr, &computeDescriptorSetLayout) != VK_SUCCESS) {
        LOGI("failed to create compute descriptor set layout!");
    }
}

void Render::createComputePipeline() {
    auto computeShaderCode = readAssetFile("comp.spv");
    ...
    if (vkCreatePipelineLayout(device, &pipelineLayoutInfo, nullptr, &computePipelineLayout) != VK_SUCCESS) {
        throw VulkanException("failed to create compute pipeline layout!");
    }
    ...
    if (vkCreateComputePipelines(device, VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &computePipeline) != VK_SUCCESS) {
        throw VulkanException("failed to create compute pipeline!");
    }
...
void Render::createComputeDescriptorSets() {
    ...
    if (vkAllocateDescriptorSets(device, &allocInfo, computeDescriptorSets.data()) != VK_SUCCESS) {
        LOGI("failed to allocate descriptor sets!");
    }
...
void Render::createComputeCommandBuffers() {
    ...
    if (vkAllocateCommandBuffers(device, &allocInfo, computeCommandBuffers.data()) != VK_SUCCESS) {
        LOGI("failed to allocate compute command buffers!");
    }
}

void Render::createShaderStorageBuffers() {
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        ...
        if (vkCreateBuffer(device, &bufferInfo, nullptr, &shaderStorageBuffers[i]) != VK_SUCCESS) {
            throw VulkanException("failed to create shader storage buffer!");
        }

        VkMemoryRequirements memRequirements;
        vkGetBufferMemoryRequirements(device, shaderStorageBuffers[i], &memRequirements);
        ...
        if (vkAllocateMemory(device, &allocInfo, nullptr, &shaderStorageBuffersMemory[i]) != VK_SUCCESS) {
            throw VulkanException("failed to allocate shader storage buffer memory!");
        }

        vkBindBufferMemory(device, shaderStorageBuffers[i], shaderStorageBuffersMemory[i], 0);
    }
}
...
void Render::createUniformBuffers() {
    ...
    for (size_t i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        ...
        if (vkCreateBuffer(device, &bufferInfo, nullptr, &uniformBuffers[i]) != VK_SUCCESS) {
            throw VulkanException("failed to create uniform buffer!");
        }
        ...
        if (vkAllocateMemory(device, &allocInfo, nullptr, &uniformBuffersMemory[i]) != VK_SUCCESS) {
            throw VulkanException("failed to allocate uniform buffer memory!");
        }
...
void Render::initializeParticles() {
    ...
    vkAllocateCommandBuffers(device, &cmdAllocInfo, &tempCommandBuffer);
    ...
    for (int i = 0; i < MAX_FRAMES_IN_FLIGHT; i++) {
        ...
        vkCmdCopyBuffer(tempCommandBuffer, stagingBuffer, shaderStorageBuffers[i], 1, &copyRegion);
    }
...
void Render::recordComputeCommandBuffer(VkCommandBuffer commandBuffer) {
    ...
    if (vkBeginCommandBuffer(commandBuffer, &beginInfo) != VK_SUCCESS) {
        throw VulkanException("failed to begin recording compute command buffer!");
    }
    ...
    if (vkEndCommandBuffer(commandBuffer) != VK_SUCCESS) {
        throw VulkanException("failed to record compute command buffer!");
    }
}
...
void Render::updateUniformBuffer(uint32_t currentImage) {
    ...
    ubo.deltaTime = deltaTime * 1000.0f;
